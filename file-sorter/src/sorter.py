...
def sort(folder_path: str, backup_enabler: bool, cancel=None, on_progress=None) -> bool:
    
    if backup_enabler:
        if not backup(folder_path, True, cancel):
            return False

    if not folder_setup(folder_path, cancel):
        return False
...
def backup(folder_path: str, allow_overwrite: bool, cancel=None) -> bool:
    if cancel and cancel.is_set():
        return False
...
def folder_setup(folder_path: str, cancel=None) -> bool:
